...
template Bagels() {
    ...
    // Checking if elements of soln matches with guess ones
    for (j=0; j<3; j++) {
        for (k=0; k<3; k++) {
            equalHB[3*j+k] = IsEqual();
            equalHB[3*j+k].in[0] <== soln[j];
            equalHB[3*j+k].in[1] <== guess[k];
            blow += equalHB[3*j+k].out;         // When any of the guess is matching
            if (j == k) {
                hit += equalHB[3*j+k].out;      // When guess is at same position as of soln
 ...
 component main {public [pubGuessA, pubGuessB, pubGuessC, pubNumHit, pubNumBlow, pubSolnHash]} = Bagels();
